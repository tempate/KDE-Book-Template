...
% Set the footnote font size to very small.
    % (Use a readable 9/11 pt instead of \scriptsize)
...
% ===== XeLaTeX fonts (book-standard trio) =====
    \usepackage{fontspec}
    \setmainfont{Libertinus Serif}   % Fallback: Linux Libertine O
    \setsansfont{TeX Gyre Heros}     % Sans for heads/running elements
    \setmonofont{Inconsolata}        % Mono for code
\fi
...
\usepackage{inconsolata} % monospaced fallback for pdfLaTeX

% =========================
% KOMA-Script font settings
% (scrbook/scrreprt/scrartcl). No titlesec.
% =========================

% Headings hierarchy suited to a 5x8" book:
% Chapter ~ \Large (~17–18pt), Section ~ \large (~12pt),
% Subsection/Subsubsection ~ \normalsize with weight/shape for contrast.
...
% Captions: small text, bold label (compatible with caption package if loaded).
\setkomafont{caption}{\small}
\setkomafont{captionlabel}{\bfseries}

% Running heads / page numbers (optional, subtle sans):
\setkomafont{pagehead}{\sffamily\small}
\setkomafont{pagenumber}{\sffamily\small}

% Lists, footnotes, etc., inherit body font; footnote size set above.
